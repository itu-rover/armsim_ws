...
class ArucoPublisher:
    ...
    def __init__(self):
        ...
        self.camera_frame = rospy.get_param("~camera_frame", "camera1")
        ...
        try:
            self.static_cam_pose = self.buffer.lookup_transform(
                "base_link", self.camera_frame, rospy.Time.now(), rospy.Duration(5.0))
    ...
    def gazebo_poses(self, msg):
        ...
        for i in range(len(msg.name)):
            ...
            elif "arm26" in msg.name[i]:
                self.robotpose = msg.pose[i]
    ...
    def update(self):
        ...
        # obtain camera and robot pose in odom space
        stat_cpos = self.static_cam_pose.transform.translation
        stat_crot = self.static_cam_pose.transform.rotation
        mag_pos = self.robotpose.position
        mag_rot = self.robotpose.orientation
        ...
        finalists = []
        # transform the marker translations into camera frame
        for fid in fiducials:
            ...
            rvec = np.array([0, 0, 0], np.float64)
